...
function createLineReader(
  ...
  const handleChunk = (chunk: Buffer | string): void => {
    ...
    for (const ch of text) {
      if (ch === "\n" || ch === "\r") {
        // \r\n on Windows: swallow the \n that follows a \r by only
        // emitting on the first terminator and leaving any trailing
        // \n to be coalesced into the next iteration as an empty
        // line. We instead treat a bare \r the same as \n and skip
        // a directly-following \n via a small look-ahead flag.
